...
export default function Navbar({ pPhone, pEmail}: NavbarProps) {
  ...
  // Navbar schrumpfen beim Scrollen
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <nav
        className={`hidden md:flex fixed top-0 left-[10%] w-[80%] z-50 transition-all bg-[var(--lightgray)] duration-[var(--transition)] ${
          isSmall ? "h-[75px]" : "h-[100px]"
        }`}
      >
        <div className="flex items-center h-full justify-between px-[10%] w-full">
          <Image
          ...
          <div className="text-right">
            <ul
              className={`flex gap-4 text-sm font-bold transition-opacity duration-300 ${
            ...
            <ul
              className={`flex gap-6 text-[20px] font-bold mt-2 justify-end transition-all duration-300 ${
          ...
          <CartIcon
            isEmpty={true}
            className={`h-[60%] w-auto cursor-pointer ${pathname === '/cart' ? 'text-[var(--accent)]' : 'text-black'} hover:text-[var(--accent)]`}
            dotColor="var(--accent)"
            onClick={() => router.push("/")}
          />
      ...
      >
        ...
        <CartIcon
          isEmpty={true}
          className={`h-[40px] w-auto cursor-pointer ${pathname === '/cart' ? 'text-[var(--accent)]' : 'text-black'}`}
          dotColor="var(--accent)"
          onClick={() => router.push("/")}
        />
